...
func PrintAllPokemon(pokemonList map[string]PokemonData) {
	var s string
	var i int
...
func ChooseTeam(input *UserInput, pokemonList map[string]PokemonData) *UserInput {
    ...
    for {
        ...
        fmt.Println()
        if mv == "1" {
            input = BuildTeam(input, pokemonList, false)
            break
        } else if mv == "2" {
            input = RandomTeam(input, pokemonList, false)
            break
        } else {
            fmt.Println("[[ INVALID INPUT ]] Try again")
            continue
...
func BuildTeam(input *UserInput, pokemonList map[string]PokemonData, makeStrong bool) *UserInput {
    ...
    for i := 0; i < 6; i++ {
        for {
            ...
            if !isValid {
                ...
                team = append(team, NewPokemon(mv, makeStrong, pokemonList))
...
func RandomTeam(input *UserInput, pokemonList map[string]PokemonData, makeStrong bool) *UserInput {
    ...
    for i := 0; i < 6; i++ {
        pokemon := allPokemon[rand.Intn(len(allPokemon))]
        team = append(team, NewPokemon(pokemon, makeStrong, pokemonList))
        printTeam = append(printTeam, pokemon)
    }
